...
class Event_Calendar_Post_Types {
    /**
     * Register custom post type
     *
    ...
    private function register_single_post_type( $fields ) {
        ...
        $labels = array(
            ...
            'new_item'              => sprintf( __( 'New %s', 'event-calendar' ), $fields['singular'] ),
            'add_new_item'          => sprintf( __( 'Add new %s', 'event-calendar' ), $fields['singular'] ),
            'edit_item'             => sprintf( __( 'Edit %s', 'event-calendar' ), $fields['singular'] ),
            'view_item'             => sprintf( __( 'View %s', 'event-calendar' ), $fields['singular'] ),
            'view_items'            => sprintf( __( 'View %s', 'event-calendar' ), $fields['plural'] ),
            'search_items'          => sprintf( __( 'Search %s', 'event-calendar' ), $fields['plural'] ),
            'not_found'             => sprintf( __( 'No %s found', 'event-calendar' ), strtolower( $fields['plural'] ) ),
            'not_found_in_trash'    => sprintf( __( 'No %s found in trash', 'event-calendar' ), strtolower( $fields['plural'] ) ),
            'all_items'             => sprintf( __( 'All %s', 'event-calendar' ), $fields['plural'] ),
            'archives'              => sprintf( __( '%s Archives', 'event-calendar' ), $fields['singular'] ),
            'attributes'            => sprintf( __( '%s Attributes', 'event-calendar' ), $fields['singular'] ),
            'insert_into_item'      => sprintf( __( 'Insert into %s', 'event-calendar' ), strtolower( $fields['singular'] ) ),
            'uploaded_to_this_item' => sprintf( __( 'Uploaded to this %s', 'event-calendar' ), strtolower( $fields['singular'] ) ),
            /* Labels for hierarchical post types only. */
            'parent_item'           => sprintf( __( 'Parent %s', 'event-calendar' ), $fields['singular'] ),
            'parent_item_colon'     => sprintf( __( 'Parent %s:', 'event-calendar' ), $fields['singular'] ),
    ...
    private function register_single_post_type_taxnonomy( $tax_fields ) {
        $labels = array(
            ...
            'all_items'                  => sprintf( __( 'All %s' , 'event-calendar' ), $tax_fields['plural'] ),
            'edit_item'                  => sprintf( __( 'Edit %s' , 'event-calendar' ), $tax_fields['single'] ),
            'view_item'                  => sprintf( __( 'View %s' , 'event-calendar' ), $tax_fields['single'] ),
            'update_item'                => sprintf( __( 'Update %s' , 'event-calendar' ), $tax_fields['single'] ),
            'add_new_item'               => sprintf( __( 'Add New %s' , 'event-calendar' ), $tax_fields['single'] ),
            'new_item_name'              => sprintf( __( 'New %s Name' , 'event-calendar' ), $tax_fields['single'] ),
            'parent_item'                => sprintf( __( 'Parent %s' , 'event-calendar' ), $tax_fields['single'] ),
            'parent_item_colon'          => sprintf( __( 'Parent %s:' , 'event-calendar' ), $tax_fields['single'] ),
            'search_items'               => sprintf( __( 'Search %s' , 'event-calendar' ), $tax_fields['plural'] ),
            'popular_items'              => sprintf( __( 'Popular %s' , 'event-calendar' ), $tax_fields['plural'] ),
            'separate_items_with_commas' => sprintf( __( 'Separate %s with commas' , 'event-calendar' ), $tax_fields['plural'] ),
            'add_or_remove_items'        => sprintf( __( 'Add or remove %s' , 'event-calendar' ), $tax_fields['plural'] ),
            'choose_from_most_used'      => sprintf( __( 'Choose from the most used %s' , 'event-calendar' ), $tax_fields['plural'] ),
            'not_found'                  => sprintf( __( 'No %s found' , 'event-calendar' ), $tax_fields['plural'] ),
        );
        $args = array(
    ...
    public function create_custom_post_type() {
         ...
         * @link https://github.com/JoeSz/WordPress-Plugin-Boilerplate-Tutorial/blob/9fb56794bc1f8aebfe04e99b15881db0c4bc61bd/event-calendar/includes/class-event-calendar-post_types.php#L230
         */
